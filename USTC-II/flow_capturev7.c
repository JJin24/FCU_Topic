...
// 取得本機介面 IP 位址
int get_iface_ip(const char *ifname, char *ip, size_t ip_len) {
    int fd;
    struct ifreq ifr;
...
void *process_packet(void *args) {
    ...
    // 只有在 packet_count < N_PKTS 時才寫入檔案
...
void send_traffic_to_api(unsigned long long bytes, int interval_seconds) {
    ...
    snprintf(postdata, sizeof(postdata), "{\"src_ip\": \"%s\", \"interval_seconds\": %d, \"bytes\": %llu}", ip, interval_seconds, bytes);
...
int main() {
    ...
    curl_global_init(CURL_GLOBAL_ALL);
        ...
        if (get_iface_ip(dev->name, ip, sizeof(ip)) != 0) {
