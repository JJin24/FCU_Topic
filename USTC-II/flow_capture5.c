...
void cleanup_flow_and_send(Flow *flow, const char *src_ip, const char *dst_ip, uint16_t src_port, uint16_t dst_port, uint8_t protocol) {
    if (flow->save_files) {
        ...
        FILE *pcap_file = fopen(full_filename, "rb");
        if (pcap_file) {
            ...
            } else if (file_size > MAX_PCAP_DATA_SIZE) {
                fprintf(stderr, "Warning: pcap file %s (%ld bytes) is larger than buffer (%d bytes). Flow will not be sent.\n", full_filename, file_size, MAX_PCAP_DATA_SIZE);
            }
            fclose(pcap_file);
        }
