...
uint8_t* get_n_byte_from_pkt(int n, const uint8_t *pkt, int pkt_len) {
    ...
    // 決定要複製的實際長度
    int copy_len = (pkt_len < n) ? pkt_len : n; //! 如果 (pkt_len < n)，只複製 pkt_len 個位元組，反之則複製 n 個位元組
...
void bytes_to_onehot_image(const uint8_t *bytes, int n, unsigned char image[256][n]) {
    //! *bytes 指向 pkt 的前 n 個位元組中的第一個 ( bytes[0] )
    //! n 代表要抓取封包的前 n 個位元組
    memset(image, 0, sizeof(unsigned char) * 256 * n); // 初始化 image 為 0
    for (int i = 0; i < n; ++i) {
        image[255 - bytes[i]][i] = 255; // 灰階最大值
    }//! one-hot的關係，在 255 個選項中只會有一個內容被點亮，而透過我們選取的 100 個bytes，
     //! 在每一個 bytes 中都有 255 種可能。所以會有 100 個元素被點亮 ( 矩陣中有100*256個元素 )
}
