...
label = ['Good', 'Bot_Attack',
         ...
         'DoS_SlowHTTPTest_Attack', 'DoS_Slowloris_Attack', 'SQL_Injection',
         'SSH_Bruteforce_Attack']
n_pkts = 6
m_bytes = 100
threshold = 0.5 # 預測機率的閾值
...
def predit(FlowDir_path, model):
    ...
    if max_prob.item() > threshold:
      result_index = pred_class.item()
      predicted_label = label[result_index]
    else:
      result_index = 15
      predicted_label = "other"

    # 顯示預測資料夾名稱、類別、索引、最大機率值
    print(f"{os.path.basename(FlowDir_path)}, {predicted_label}, {result_index}, Score: {max_prob.item():.4f}")

    # print(f"資料夾: {os.path.basename(FlowDir_path)}, 預測結果: {predicted_label}, 索引: {result_index}")

    # CSV 輸出
    #print(f"{os.path.basename(FlowDir_path)}, {predicted_label}, {result_index},{torch.argmax(result, dim=1)}")
